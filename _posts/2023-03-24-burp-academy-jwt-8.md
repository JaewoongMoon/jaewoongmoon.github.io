---
layout: post
title: "Burp Academy-JWT 여덞번째 문제: 알고리즘 컨퓨전을 통한 JWT 인증우회"
...
- 그러나 [rsa_sign2n](https://github.com/silentsignal/rsa_sign2n)을 사용하면 발행된 토큰에서 부터 공개키를 얻어내는 것이 가능하다고 한다. 물론 서버측의 토큰 발행 구현 부분에 보안상 결함이 있는 경우에 가능하다. 
...
이 중에서 `Base64 encoded x509 key` 또는 `Base64 encoded pkcs1 key` 가 공개키로 보인다. x509 포맷을 사용해보자. 
...
다이얼로그에서 Generate 버튼을 눌러서 새로운 키를 JWK 포맷으로 생성한다.  `k`파라메터를 위에서 툴을 돌려서 얻어낸 x509 키로 대체하고 저장한다. 
